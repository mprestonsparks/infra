    ...
    try:
        ...
        db.execute(
            text("""
                ...
                VALUES (:id, :agent_id, :pattern_hash, 'genome', CAST(:content AS jsonb),
                        0.5, NOW())
            """),
            {
        ...
        if request.repository_url:
            try:
                ...
                import subprocess
                agent_repo_path = Path(f"/app/repositories/{agent_id}")
                agent_repo_path.parent.mkdir(parents=True, exist_ok=True)
                
                # Clone the repository
                clone_result = subprocess.run(
                    ["git", "clone", "--depth", "1", request.repository_url, str(agent_repo_path)],
                    capture_output=True,
                    text=True
                )
                
                if clone_result.returncode == 0:
                    worktree_path = agent_repo_path
                    logger.info(f"Cloned repository for agent {agent_id} to {worktree_path}")
                else:
            ...
            db.execute(
                text("""
                    ...
                    VALUES (:id, :agent_id, :pattern_hash, :pattern_type, 
                            CAST(:content AS jsonb), :effectiveness, NOW())
...
async def startup_event():
    ...
    try:
        ...
        try:
            ...
            # Apply simulate method patch for API compatibility
            import sys
            sys.path.append('/app')
